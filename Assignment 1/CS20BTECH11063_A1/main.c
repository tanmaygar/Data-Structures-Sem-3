...
{
    //If song is found in the Linked list then it enqueues it to the queue
    if(find_linked_list_node(lln, s) == true)
    {
        enq_queue_header(Q, s);
        printf("Song Found, Enqueued\n");
    }
    //Else it prints that song is not found
    else
    {
...
bool play_next(struct queue_header* Q, struct stack_node** sn)
{
    ...
    //If playlist is not empty then it pushes the song in the stack
...
int main()
{
    ...
    while(true)
    {
        ...
        //scanf("%s", input_songs);
        fgets(input_songs, STRING_SIZE, stdin);
        string_corrector(input_songs);
    ...
    while(strcmp(input_songs_playlist, input_terminate) != 0)
    {
        ...
        fgets(input_songs_playlist, STRING_SIZE, stdin);
        string_corrector(input_songs_playlist);
