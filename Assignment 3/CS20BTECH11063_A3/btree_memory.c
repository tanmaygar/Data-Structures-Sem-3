...
{
    int i = 0;
    while (secondary_memory[i] >= 2 * MEM_CELL_T_BTREE - 1)
    {
        i++;
    }
    return i;
...
{
    for (int i = 0; i < 2 * MEM_CELL_T_BTREE; i++)
    {
...
{
    //If the nodes lie in same row then FAST_BTREE
    if (x->coord[0] == y->coord[0])
    {
        (*time) = (*time) + FAST_BTREE;
    }
    //Else it does DISK READ AND FAST_BTREE
    else
    {
        (*time) = (*time) + DISK + FAST_BTREE;
    }
}
...
void time_adder_DISK_READ_3_nodes(struct BTREE_node *x, struct BTREE_node *y, struct BTREE_node *z, int *time)
{
    ...
    {
        (*time) = (*time) + DISK + DISK + FAST_BTREE + FAST_BTREE;
    }
    //If all of them are in same row
    else if (x->coord[0] == y->coord[0] && x->coord[0] == z->coord[0] && y->coord[0] == z->coord[0])
    {
        (*time) = (*time) + FAST_BTREE + FAST_BTREE;
    }
    //If two of them are in same row
    else
    {
        (*time) = (*time) + DISK + FAST_BTREE + FAST_BTREE;
    }
}
...
{
    for(int i = x->num_keys; i < 2 * MINIMUM_DEGREE - 1; i++)
    {
        x->key[i] = 0;
    }
}
...
{
    for(int i = x->num_keys + 1; i < 2 * MINIMUM_DEGREE; i++)
    {
        x->children[i] = NULL;
    }
}
...
void BTREE_node_split_child(struct BTREE_node *node, int index, int *time, int *secondary_memory)
{
    ...
    node->key[index] = y->key[(MINIMUM_DEGREE-1)];
    ...
    if (secondary_memory[node->coord[0]] < 2 * MEM_CELL_T_BTREE - 1)
    {
        ...
        //FAST_BTREE READ for access to the cell
        (*time) = (*time) + FAST_BTREE;
    }
    ...
    else
    {
        ...
        //DISK READ and FAST_BTREE READ for access to the cell
        (*time) = (*time) + FAST_BTREE + DISK;
    }
...
void insert_node_BTREE(struct BTREE_node **node, int num, int *time, int parent_coord[COORD], int *secondary_memory)
{
    ...
    if ((*node)->num_keys == 2 * MINIMUM_DEGREE - 1)
    {
        ...
        if (secondary_memory[parent_coord[0]] < 2 * MEM_CELL_T_BTREE - 1)
        {
        ...
        else
        {
            ...
            //DISK READ AND FAST_BTREE to access the cell and then DISK WRITE
            (*time) = (*time) + FAST_BTREE + DISK + DISK + DISK;
        }
        ///////////////////////////////////////////////////////////////
        ...
        else
        {
            ...
            else if (j == 0)
            {
                ...
                if ((*node)->children[1]->num_keys >= MINIMUM_DEGREE)
                {
                    ...
                    make_rest_keys_zero((*node)->children[1]);
                    make_rest_children_null((*node)->children[1]);
                ...
                else
                {
                    ...
                    //Combining the 1st and 0th child along with the first key of the node into a single child
                    struct BTREE_node *y = (*node)->children[0];
                    y->key[y->num_keys] = (*node)->key[0];
                    for (int i = 0; i < (*node)->children[1]->num_keys; i++)
                    {
            ...
            else if (j == (*node)->num_keys)
            {
                ...
                if ((*node)->children[j - 1]->num_keys >= MINIMUM_DEGREE)
                {
                    ...
                    make_rest_keys_zero((*node)->children[j - 1]);
                    make_rest_children_null((*node)->children[j - 1]);
                    //Recurses to delete the key in the last child
                    delete_node_BTREE(&(*node)->children[j], num, time, secondary_memory);
                }
                else
                {
                    //IF J=LAST and J=SECOND LAST BOTH HAVE ONLY T-1 KEYS THEN MERGE AND DELETE FROM COMBINED NODES AND PROCEED WITH CASE 3B

                    time_adder_DISK_READ_3_nodes((*node), (*node)->children[j - 1], (*node)->children[j], time);
                    time_adder_DISK_WRITE_3_nodes((*node), (*node)->children[j - 1], (*node)->children[j], time);

                    //Merging the last child and second last child with the last key into a single child
