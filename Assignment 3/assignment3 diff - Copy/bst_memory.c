...
struct BST_mem_node *search_mem_node(struct BST_mem_node *node, int key, int *time)
{
    ...
    else if (key < node->value)
    {
        ...
        if (node->left != NULL)
        {
            time_adder_DISK_READ_2_nodes_BST(node, node->left, time);
    ...
    {   
        ...
        time_adder_DISK_READ_2_nodes_BST(node, node->right, time);
...
struct BST_mem_node *insert_node_BST_mem(struct BST_mem_node **node, int key, int *time, int parent_coord[COORD], int *secondary_memory)
{
    ...
    if ((*node) == NULL)
    {
        ...
        else
        {
            ...
            //Do FAST for accessing that particular cell and DISK READ for loading that row and then perform 2 DISK WRITES for saving
