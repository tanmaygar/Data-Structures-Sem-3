...
struct AVL_TREE_node *search_key_AVL(struct AVL_TREE_node *node, char *key)
{
    //If node is NULL it returns NULL
    if (node == NULL)
    {
...
struct AVL_TREE_node *delete_node_AVL(struct AVL_TREE_node **node, char *s)
{
    ...
    else
    {
        ...
        (*node)->right = delete_node_AVL(&(*node)->right, tmp->song_name);
    }
