...
struct BST_node* make_bst_tree(char song_list_preorder[STRING_SIZE][STRING_SIZE], int *preorder_position_number, int begin, int end, int size)
{
    ...
    //index to find the first element such that the song_name of node lies before the song_name in the array
    int index;
...
void num_songs_playlist_BST(struct BST_node *node, int* num_song_playlist)
{
    ...
    num_songs_playlist_BST(node->left, num_song_playlist);
    (*num_song_playlist)++;
    num_songs_playlist_BST(node->right, num_song_playlist);
